...
for dbname in ["postgres", "onehr"]:
    print(f"--- {dbname} ---")
    try:
        conn = pg8000.native.Connection(host="hr.cx00uaqeg0tv.us-east-2.rds.amazonaws.com", database=dbname, user=secret['username'], password=secret['password'], port=5432)
